...
if [ "$1" == "healthcheck" ]
then
    docker-compose run interop-server python3.6 healthcheck.py --postgres_host interop-db --check_postgres --check_homepage
fi

# Creates the database. Only needs to be done once.
if [ "$1" == "create_db" ]
then
    docker-compose run interop-server python3.6 healthcheck.py --postgres_host interop-db --check_postgres
    docker-compose run interop-server psql "postgresql://postgres:postgres@interop-db" -c "CREATE DATABASE auvsi_suas_db;"
    docker-compose run interop-server python3.6 manage.py migrate
fi
...
then
    docker-compose up -d
    docker-compose exec interop-server python3.6 healthcheck.py --postgres_host interop-db --check_postgres --check_homepage
fi
...
if [ "$1" == "test" ]
then
    docker-compose run interop-server python3.6 manage.py test --parallel
fi
